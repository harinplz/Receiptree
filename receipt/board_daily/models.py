...
class D_Comment(models.Model):
    D_board_no = models.ForeignKey('D_Board', on_delete=models.CASCADE, db_column='D_board_no')
    D_name = models.ForeignKey('account.User', on_delete=models.CASCADE, db_column='name')
    D_body = models.TextField()
    D_date = models.DateTimeField(default=timezone.now)
